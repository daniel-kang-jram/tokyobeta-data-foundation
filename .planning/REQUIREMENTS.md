...
- [ ] `EVD-REFRESH-003` KPI cards include RENT, RevPAR, and RecPAR(Cash) when source data quality allows.
- [ ] `EVD-REFRESH-004` Every KPI/chart/table shows explicit time basis (`as_of_date` or period) and freshness metadata.
...
- [ ] `EVD-DQ-001` Occupancy logic handles edge cases (e.g., same-day move-out interpretation) and is documented.
- [ ] `EVD-DQ-002` KPI definitions are versioned and traceable to SQL/model logic.
- [ ] `EVD-DQ-003` Benchmark checks include known reference points from Feb 2026 communications:
