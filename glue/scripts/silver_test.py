"""
AWS Glue ETL Job: Silver Test Runner
Runs ONLY dbt tests for silver layer (separated from transformation for performance).

This job can run:
1. After silver_transformer completes (validation)
2. On a separate schedule (e.g., once per day instead of every run)
3. Ad-hoc for debugging
"""
...
def run_dbt_silver_tests(dbt_project_path, target_env):
    ...
    cmd = [
        ...
        "--project-dir", dbt_project_path,
        "--target", target_env,
        "--models", "silver.*"  # Only silver layer tests
    ]
...
def main():
    """Main silver test workflow."""
    start_time = datetime.now()
    
    try:
        print("="*60)
        print("SILVER TEST JOB STARTED")
        ...
        print(f"\n{'='*60}")
        print("SILVER TEST JOB COMPLETED")
        ...
        print(f"\nERROR: Silver test job failed: {str(e)}")
        import traceback
        traceback.print_exc()
